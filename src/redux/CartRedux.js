...
const mapCheckoutErrorMessage = error => {
  if (!error) {
    return Languages.CheckoutFailedTryAgain;
  }
  ...
  switch (error.type) {
    ...
    case 'HTTP_ERROR':
      return error.status >= 500
        ...
        : error.message || Languages.CheckoutFailedTryAgain;
    default:
      return error.message || Languages.CheckoutFailedTryAgain;
  }
};
...
export const actions = {
  ...
  createNewOrder: async (dispatch, payload) => {
    ...
    dispatch({
      type: types.CREATE_NEW_ORDER_ERROR,
      message: Languages.CheckoutFailedTryAgain,
    });
  },
